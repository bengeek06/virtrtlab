...
| `overruns` | u64 | Bytes dropped due to buffer overflow (oldest byte evicted) |
| `drops` | u64 | Bytes discarded by fault injection (`drop_rate_ppm`) |
...
| `enabled` ← `0` while AUT has `/dev/ttyVIRTLABx` open | raise `SIGPIPE` on next AUT write |
