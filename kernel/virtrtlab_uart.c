...
/*
 * TODO(issue #2): Register uart0 device on virtrtlab_bus with sysfs attributes
 *
 * Architecture decision:
 *   Use device_register() with dev->parent = virtrtlab_devices_kobj so that
 *   the device appears at /sys/kernel/virtrtlab/devices/uart0/ (primary path)
 *   and the bus subsystem automatically creates the symlink at
 *   /sys/bus/virtrtlab/devices/uart0. No manual kobject tree management needed.
 *   rmmod virtrtlab_core with uart loaded is refused by the bus framework
 *   automatically (device holds a reference on the bus module).
 *
 * Step 1 — virtrtlab_core.h / virtrtlab_core.c
 *   Export virtrtlab_devices_kobj so uart can set it as device parent.
 *   Add to virtrtlab_core.h:
 *     extern struct kobject *virtrtlab_devices_kobj;
 *   Add to virtrtlab_core.c after kobject_create_and_add("devices", ...):
 ...
 *       char     fault_policy[16];
 *       // UART-specific fields
 *       u32      baud;
 *       char     parity[8];  // "none" | "even" | "odd"
 *       u8       databits;   // 5 | 6 | 7 | 8
 *       u8       stopbits;   // 1 | 2
 *       // Lock protecting all rw fields from concurrent sysfs access
 *       struct mutex lock;
 *   };
 *   Allocate with kzalloc(GFP_KERNEL), free in dev->release().
 *
 * Step 3 — device_register()
 *   dev->bus     = &virtrtlab_bus_type;
 *   dev->parent  = virtrtlab_devices_kobj->parent (the kobj itself as parent
 *                  requires casting; use device_initialize() + dev_set_name()
 *                  + device_add() for fine-grained control).
 *   dev->release = virtrtlab_uart_dev_release;  // calls kfree()
 *   dev->groups  = uart0_groups;                // sysfs attribute groups
 *   dev_set_name(&uart_dev->dev, "uart0");
 *   device_register(&uart_dev->dev);
 *
 * Step 4 — Common sysfs attributes
 *   DEVICE_ATTR_RO(type)         -> returns "uart\n"
 *   DEVICE_ATTR_RO(bus)          -> returns "vrtlbus0\n"
 *   DEVICE_ATTR_RW(enabled)      -> validate: "0" or "1"
 *   DEVICE_ATTR_RW(mode)         -> validate: "normal"|"record"|"replay"
 *   DEVICE_ATTR_RW(latency_ns)   -> validate: kstrtou32(), any value
 *   DEVICE_ATTR_RW(jitter_ns)    -> validate: kstrtou32()
 *   DEVICE_ATTR_RW(drop_rate_ppm)    -> validate: kstrtou32(), <= 1000000
 *   DEVICE_ATTR_RW(bitflip_rate_ppm) -> validate: kstrtou32(), <= 1000000
 *   DEVICE_ATTR_RW(fault_policy) -> validate: string enum
 *
 * Step 5 — UART-specific sysfs attributes
 *   DEVICE_ATTR_RW(baud)      -> validate: kstrtou32(), > 0
 *   DEVICE_ATTR_RW(parity)    -> validate: "none"|"even"|"odd"
 *   DEVICE_ATTR_RW(databits)  -> validate: kstrtou8(), in {5,6,7,8}
 *   DEVICE_ATTR_RW(stopbits)  -> validate: kstrtou8(), in {1,2}
 *
 * Step 6 — Attribute groups
 *   static struct attribute *uart0_common_attrs[] = { ..., NULL };
 *   static struct attribute *uart0_uart_attrs[]   = { ..., NULL };
 *   static const struct attribute_group uart0_common_group = { ... };
 *   static const struct attribute_group uart0_uart_group   = {
 *       .name  = "uart",   // creates /sys/.../uart0/uart/ subdirectory
 *       .attrs = uart0_uart_attrs,
 *   };
 *   static const struct attribute_group *uart0_groups[] = {
 *       &uart0_common_group, &uart0_uart_group, NULL
 *   };
 *
 * Step 7 — module init / exit
 *   init: kzalloc → mutex_init → device_register
 *   exit: device_unregister (triggers release via put_device refcount → 0)
 */
